...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "for":
        ...
        while True:
            cond_value, status = evaluate(ast["condition"], environment)
            if status == "exit": return cond_value, "exit"
            if not is_truthy(cond_value): break

            # execute body
            result, status = evaluate(ast["body"], environment)
            ...
            # update
            _, status = evaluate(ast["update"], environment)
