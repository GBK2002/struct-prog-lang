...
grammar = """
    ...
    for_statement = "for" "(" statement ";" expression ";" statement ")" statement_list
...
def parse_for_statement(tokens):
    assert tokens[0]["tag"] == "for", "Expected 'for'"
    tokens = tokens[1:]
    assert tokens[0]["tag"] == "(", "Expected '(' after for"
    tokens = tokens[1:]

    init_stmt, tokens = parse_statement(tokens)
    ...
    update_stmt, tokens = parse_statement(tokens)
